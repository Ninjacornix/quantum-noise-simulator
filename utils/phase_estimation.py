...
class PhaseEstimation:
    def __init__(self, n=2, state=None, noise=False, noise_type=None, noise_options=None, measure=False, comparison=False):
        self.n = n
        self.state = Statevector.from_label(state) if state else None
        self.qc = None
        self.noise = noise
    ...
    def build_phase_estimation(self):
        qc = QuantumCircuit(self.n + 1)
        if self.state is not None:
            qc.initialize(self.state, range(self.n))
        self.qc = self.add_phase_estimation_circuit(qc)
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
            circuit_drawer(qc, output="mpl", filename=tmp.name, style="iqx-standard")
            img_path = tmp.name
        return img_path

    def add_phase_estimation_circuit(self, qc):
        for i in range(self.n):
            qc.h(i)
        qc.x(self.n)
        for i in range(self.n):
            for _ in range(2 ** i):
                qc.cp(np.pi / 4, i, self.n)
        qc.barrier()
        for qubit in range(self.n // 2):
            qc.swap(qubit, self.n - qubit - 1)
            qc.barrier()
        def add_iqft_rotations_to_controls(circuit, n):
            if n == 0:
                return circuit
            n -= 1
            ...
            circuit.h(n)
            circuit.barrier()
            add_iqft_rotations_to_controls(circuit, n)
        add_iqft_rotations_to_controls(qc, self.n)
        if self.noise:
    ...
    def simulate(self):
        ...
        if self.noise:
            ...
            if self.noise_type == "Depolarizing":
                ...
                noise_model.add_all_qubit_quantum_error(noise_ops, ['h', 'x'])
